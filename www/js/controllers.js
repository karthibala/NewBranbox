...
.controller('gallery', function($scope,$http) {
    ...
    //alert("Gallery view");
	 ...
	 (function() {
		...
		initPhotoSwipeFromDOM('.demo-gallery'); 
		//initPhotoSwipeFromDOM("div[id^='demo']");
		

	})();
	
	/* PhotoSwipe script Ends */
   ...
   // $('#lightgallery').lightGallery();
    ...
    db.transaction(function(tx){
		...
		// tx.executeSql("SELECT * FROM  gallery", [], function(tx, results) {
           ...
           // var gallerydatas=results.rows;
           // for(var i = 0; i < results.rows.length; i++)
           // {
               // var row = gallerydatas.item(i);
               // json_gallery.push(row);
           // }
           // $scope.$apply(function() {
               // $scope.galleryNewData=json_gallery;
           // });
		// });
		
		/* ng-repeat ends */
		
		/* Append Query Function Start */
		...
		tx.executeSql("SELECT * FROM  album", [], function(tx, results) {
           ...
           {
				var row = albumdatas.item(i);
				$('#setGallery').append("<h3>"+row.galName+"</h3><div class='myClass' id="+row.galId+"></div>");
				    var galID=row.galId;
					tx.executeSql("SELECT * FROM gallery where galId=?", [galID], function(tx, results) {
					  // alert("gallery->"+results.rows.length);
					   var gallerydatas=results.rows;
					   for(var i = 0; i < results.rows.length; i++)
					   {
						   var row = gallerydatas.item(i);
						   //alert("galleryId"+row.galId);
						   $('#'+row.galId).append('<a href='+ row.images +' data-size="1600x1600" data-med='+ row.images +' data-med-size="1024x1024" data-author="Folkert Gorter"><img src='+ row.images +' alt="" /></a>');
					   }
					});
           }
           $('.myClass').addClass('demo-gallery');
		});
		...
		tx.executeSql("SELECT * FROM video", [], function(tx, results) {
		   ...
		   var video=results.rows;
		   $('#videoPath').prepend('<h3>Videos</h3><div id="videoPlayer"></div>');
		   for(var i = 0; i < results.rows.length; i++)
		   {
			   var row = video.item(i);
			   //alert(row.url);
			   $('#videoPlayer').append('<video style="height: 100%; width: 100%" poster="img/profile-menu.png" controls="controls" preload="auto" data-setup="{}"><source type="video/mp4" src='+ row.url +' /></video>');
			   
			   //$('#videoPath').append('<video style="height: 100%; width: 100%" poster="img/profile-menu.png" controls="controls" preload="auto" data-setup="{}"><source type="video/mp4" src='+ row.url +' /></video>');
		   }
		});
		...
		// function showImage(id){
			// var data = id;
			// alert(data);
			// tx.executeSql("SELECT * FROM  gallery where galId=?", [data], function(tx, results) {
				   // alert("gallery->"+results.rows.length);
				   // var gallerydatas=results.rows;
				   // for(var i = 0; i < results.rows.length; i++)
				   // {
					   // var row = gallerydatas.item(i);
					   // $('#Testing').append('<a href='+ row.images +' data-size="1600x1600" data-med='+ row.images +' data-med-size="1024x1024" data-author="Folkert Gorter" class="demo-gallery__img--main"><img src='+ row.images +' alt="" width="150px"; height="150px"; /></a>'); 
				   // }
				// });
		// }
		
		
		/* Append Query Function End */
...
.controller('registerForm', function($scope,$http,$location,$cordovaOauth,$ionicPlatform,$cordovaDatePicker,$cordovaDialogs) { 
    ...
    $scope.insertForm=function()
    {
        ...
        .success(function (data) {
			...
			/*	Email verfication send ajax start */
				...
				/*	Email verfication send ajax end */
...
.controller('authentication', function($scope,$http,$location,$cordovaOauth,$cordovaDialogs,$cordovaFileTransfer) {  
  ...
  $scope.loginAuthentication=function()
  {
    ...
    var businessId= localStorage.getItem("businessId");
	var regid=localStorage.getItem("regid");
	//var id = prompt("regid", regid);
    $http.post('http://branboxadmin.elasticbeanstalk.com/branboxApp/ajaxLogin_Test.php',{password:password,email:email,regid:regid}, {headers: {'Content-Type': 'application/x-www-form-urlencoded'} })
    .success(function (json) {
      var ajaxlength = json.length;
      //alert("Login start-"+ajaxlength);
     // console.log(json);
        if(ajaxlength==1)
        {
                    ...
                    $http.get("https://graph.facebook.com/v2.2/me", { params: { access_token: result.access_token, fields: "id,name,gender,email,location,website,picture,relationship_status", format: "json" }}).then(function(result) {
                          ...
                          for( var i=0; i < 5; i++ ) {
                              ...
                              .success(function (data) {
                                ...
                                $cordovaDialogs.alert('Verification Link attatched in registered Mail', 'Please Check Your Mail', 'OK')
                                    .then(function() {
                                });
                              //swal({  
                              //        title: "Please Check Your Mail",   
